...
class _TransactionsScreenState extends State<TransactionsScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _performSmsSync() async {
    ...
    try {
      final result = await TransactionSyncService.syncTransactions();
      final count = result["syncedCount"] ?? 0;
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(count > 0 
                ? "Synced! Loaded $count new transaction(s)." 
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      ...
      builder: (context, child) {
        ...
        final smsCount = state.transactions.where((tx) => tx.sender != "MANUAL" && tx.sender != "CSV" && tx.sender != "PDF").length;
        final csvCount = state.transactions.where((tx) => tx.sender == "CSV").length;
        final pdfCount = state.transactions.where((tx) => tx.sender == "PDF").length;
        ...
        final filteredList = state.transactions.where((tx) {
          ...
          if (_activeFilter == "SMS") return tx.sender != "MANUAL" && tx.sender != "CSV" && tx.sender != "PDF";
          if (_activeFilter == "CSV") return tx.sender == "CSV";
          if (_activeFilter == "PDF") return tx.sender == "PDF";
          ...
          floatingActionButton: state.transactions.isEmpty
              ...
              : AnimatedScale(
                  ...
                  child: FloatingActionButton(
                    onPressed: () {
                      HapticFeedback.mediumImpact();
                      _showAddOptionsSheet(context, isDark, state.smsTrackingEnabled);
                    },
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    _buildFilterChip("PDF", pdfCount, isDark),
                  ],
                ),
              ),
              const SizedBox(height: 16),
                                ...
                                children: [
                                  ...
                                  Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: items.map((tx) => _buildSwipeableRow(context, tx, isDark)).toList(),
                                    ),
                                  ),
                                ],
                              );
          ...
          gradient: isSelected
              ? (isDark
                  ? const LinearGradient(
                      colors: [AppColors.accentNeon, Color(0xFF00C853)],
                      begin: Alignment.topLeft,
                      end: Alignment.bottomRight,
                    )
                  : const LinearGradient(
                      colors: [Color(0xFF0F6038), Color(0xFF1B8D56)],
                      begin: Alignment.topLeft,
                      end: Alignment.bottomRight,
                    ))
              : null,
          color: isSelected ? null : (isDark ? AppColors.darkCardBg : AppColors.lightCardBg),
  ...
  Widget _buildSwipeableRow(BuildContext context, TransactionModel tx, bool isDark) {
    final uniqueKey = Key("tx_${tx.id ?? tx.fingerprint}");
  ...
  Widget _buildTransactionRowItem(BuildContext context, TransactionModel tx, bool isDark) {
    ...
    final isSMS = tx.sender != "MANUAL" && tx.sender != "CSV" && tx.sender != "PDF";
    final isCSV = tx.sender == "CSV";
    final isPDF = tx.sender == "PDF";
    final borderColor = isDark ? Colors.white.withOpacity(0.08) : AppColors.lightBorder;
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        if (isSMS) ...[
                          const SizedBox(width: 6),
                          _buildBadge("SMS", isDark ? const Color(0xFF0A2B18) : const Color(0xFFE8F5E9), isDark ? AppColors.accentNeon : AppColors.lightGradient[0]),
                        ] else if (isCSV) ...[
                          const SizedBox(width: 6),
                          _buildBadge("CSV", isDark ? const Color(0xFF0C1929) : const Color(0xFFE3F2FD), Colors.blue),
                        ] else if (isPDF) ...[
                          const SizedBox(width: 6),
                          _buildBadge("PDF", isDark ? const Color(0xFF2C0F14) : const Color(0xFFFFEBEE), Colors.redAccent),
                        ],
                      ],
                    ),
  ...
  // Options sheet for FAB tapping
  void _showAddOptionsSheet(BuildContext context, bool isDark, bool smsTrackingEnabled) {
    final primaryColor = isDark ? AppColors.accentNeon : AppColors.lightGradient[0];
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: Icon(
                            smsTrackingEnabled ? Icons.payments_rounded : Icons.edit_note_rounded,
                            color: primaryColor,
                            size: 24,
                          ),
                        ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Text(
                                smsTrackingEnabled ? "Record Cash Payment" : "Add Transaction Manually",
                                style: textStyleTitle,
                              ),
                              const SizedBox(height: 4),
                              Text(
                                smsTrackingEnabled
                                    ? "Add a cash transaction that is not tracked via SMS"
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text(
                                "Import transaction history from a CSV or PDF file",
                                style: textStyleSub,
                              ),
                            ],
                          ),
